...
MapController = function(map) {
  ...
  this.spawnMob = function() {
    ...
    var rand = Math.floor(Math.random()*(this.spawn.mobs[i].location.length-1));
    var loc = this.spawn.mobs[i].location[rand];
    ...
    console.log(new Mob(this.spawn.mobs[i].species, pos));
